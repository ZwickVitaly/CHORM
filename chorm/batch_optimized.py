...
class ClickHouseBatchInsert:
    ...
    def add_row(self, row: Sequence[Any]) -> bool:
        ...
        if self.columns and len(row) != len(self.columns):
            raise ValueError(
                f"Row has {len(row)} values but {len(self.columns)} columns expected"
            )
    ...
    def flush(self) -> int:
        ...
        logger.info(
            f"Inserted batch #{self._batches_sent}: {row_count} rows "
...
class ClickHouseBatchInsertFromDataFrame:
    ...
    def insert_dataframe(self, df: Any) -> Dict[str, Any]:
        ...
        for i in range(0, total_rows, self.batch_size):
            ...
            logger.info(
                f"Inserted batch #{self._batches_sent}: {batch_size} rows "
                f"into {self.table_name}"
            )
        ...
        logger.info(
            f"DataFrame insert complete: {stats['total_rows']} rows "
            f"in {stats['batches_sent']} batches"
        )
...
def bulk_insert(
    ...
    if hasattr(data, 'iloc'):  # pandas DataFrame
        batch = ClickHouseBatchInsertFromDataFrame(
            ...
            optimize_on_finish=optimize_on_finish
        )

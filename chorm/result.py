    ...
    def one(self) -> Row:
        ...
        if not rows:
            raise NoResultFound(
                ...
                "Use .first() or .one_or_none() if this is expected."
            )
...
class ScalarResult(Generic[T]):
    ...
    def one(self) -> Any:
        ...
        if not all_res:
            raise NoResultFound(
                "Query returned no results. "
                "Use .first() or .scalar_one_or_none() if this is expected."
            )
        raise MultipleResultsFound(
            f"Query returned {len(all_res)} results, expected 1. "
            f"Use .first() or add .limit(1) to your query."
        )
    ...
    def one_or_none(self) -> Optional[Any]:
        ...
        if not all_res:
            return None
        raise MultipleResultsFound(
            f"Query returned {len(all_res)} results, expected 0 or 1. "
            f"Use .first() or add .limit(1) to your query."
        )

...
class RangeValidator(Validator):
    ...
    def __call__(self, value: Any, column_name: str | None = None) -> Any:
        ...
        if self.min_value is not None and num_value < self.min_value:
            raise ValidationError(
                f"Value {value!r} is less than minimum {self.min_value}",
                column_name,
                value
            )
        ...
        if self.max_value is not None and num_value > self.max_value:
            raise ValidationError(
                f"Value {value!r} is greater than maximum {self.max_value}",
                column_name,
                value
            )
...
class LengthValidator(Validator):
    ...
    def __call__(self, value: Any, column_name: str | None = None) -> Any:
        ...
        if not isinstance(value, (str, bytes, list, tuple)):
            raise ValidationError(
                f"Value {value!r} is not a string or sequence",
                column_name,
                value
            )
        ...
        if self.min_length is not None and length < self.min_length:
            raise ValidationError(
                f"Value {value!r} length {length} is less than minimum {self.min_length}",
                column_name,
                value
            )
        ...
        if self.max_length is not None and length > self.max_length:
            raise ValidationError(
                f"Value {value!r} length {length} is greater than maximum {self.max_length}",
                column_name,
                value
            )
...
class RegexValidator(Validator):
    ...
    def __call__(self, value: Any, column_name: str | None = None) -> Any:
        ...
        if not self.pattern.match(value):
            raise ValidationError(
                f"Value {value!r} does not match pattern {self.pattern.pattern!r}",
                column_name,
                value
            )
...
class EmailValidator(Validator):
    ...
    def __call__(self, value: Any, column_name: str | None = None) -> Any:
        ...
        if '@' not in value:
            raise ValidationError(
                f"Value {original_value!r} is not a valid email address (missing @)",
                column_name,
                original_value
            )
        ...
        if not self.EMAIL_PATTERN.match(value):
            raise ValidationError(
                f"Value {original_value!r} is not a valid email address format",
                column_name,
                original_value
            )
...
class InValidator(Validator):
    ...
    def __call__(self, value: Any, column_name: str | None = None) -> Any:
        ...
        if value not in self.choices:
            raise ValidationError(
                f"Value {value!r} is not in allowed choices: {self.choices}",
                column_name,
                value
            )
...
class NotInValidator(Validator):
    ...
    def __call__(self, value: Any, column_name: str | None = None) -> Any:
        ...
        if value in self.choices:
            raise ValidationError(
                f"Value {value!r} is in disallowed choices: {self.choices}",
                column_name,
                value
            )

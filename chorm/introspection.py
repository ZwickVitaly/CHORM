...
class ModelGenerator:
        ...
        if (
            ...
            if match:
                ...
                if single_arg_match:
                    ...
                    return f"Decimal({precision}, {scale})"
                return "Decimal(18, 2)  # TODO: Verify precision/scale"
    ...
    def generate_model(self, table_info: Dict[str, Any]) -> str:
        ...
        if table_info["engine"] == "MaterializedView":
            ...
            if to_table:
                ...
                to_table_ref = table_map.get(clean_to_table, f'"{to_table}"')
            else:
                to_table_ref = None
            ...
            if not to_table:
                ...
                engine_match = re.search(
                    r"\sENGINE\s*=\s*(.+?)(?:\s+(?:POPULATE|AS\s+SELECT|SETTINGS))",
                    create_query,
        ...
        for col in table_info["columns"]:
            ...
            if col.get("codec"):
                ...
                if codec_val:
                    ...
                    codec_arg = f", codec={codec_expr}" if codec_expr else f', codec="{codec_val}"'
    ...
    def _map_distributed_engine(self, engine_full: str) -> str:
        ...
        inner = (
            engine_full[len("Distributed(") : -1] if engine_full.endswith(")") else engine_full[len("Distributed(") :]
        )
    ...
    def _parse_codec_expression(self, codec_str: str) -> Optional[str]:
        ...
        known_codecs = {"ZSTD", "LZ4", "LZ4HC", "Delta", "DoubleDelta", "Gorilla", "T64", "FPC", "NONE"}
    ...
    def generate_file(self, tables_info: List[Dict[str, Any]]) -> str:
        ...
        self.table_to_class: Dict[str, str] = {t["name"]: self._to_class_name(t["name"]) for t in tables_info}

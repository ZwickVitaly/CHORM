...
def test_group_by_multiple_columns():
    ...
    stmt = (
        select(User.city, User.country, func.count(User.id))
...
def test_group_by_with_expression():
    ...
    stmt = (
        select(func.toYYYYMM(Order.user_id), func.sum(Order.amount))
        .group_by(func.toYYYYMM(Order.user_id))
    )
    sql = stmt.to_sql()
...
def test_group_by_with_where():
    ...
    stmt = (
        select(Order.status, func.sum(Order.amount))

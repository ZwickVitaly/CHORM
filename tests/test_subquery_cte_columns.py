...
class TestSubqueryColumnAccess:
    ...
    def test_subquery_c_attribute_labeled_columns(self):
        ...
        ).group_by(User.id).subquery('user_stats')
    ...
    def test_two_level_subquery_nesting(self):
        ...
        ).group_by(Order.user_id).subquery('daily')
        
        # Level 2: Use daily.c to reference columns
        monthly = select(
            daily.c.user_id,
            func.avg(daily.c.daily_total).label('avg_daily')
        ).group_by(daily.c.user_id).subquery('monthly')
    ...
    def test_three_level_subquery_nesting(self):
        ...
        level1 = select(
            Order.user_id,
            func.sum(Order.amount).label('total')
        ).group_by(Order.user_id).subquery('l1')
        
        # Level 2
        level2 = select(
            level1.c.user_id,
            func.avg(level1.c.total).label('avg_total')
        ).group_by(level1.c.user_id).subquery('l2')
        
        # Level 3
        level3 = select(
            level2.c.user_id,
            level2.c.avg_total
        ).where(level2.c.avg_total > 100).subquery('l3')
...
class TestMixedCTESubquery:
    ...
    def test_cte_and_subquery_together(self):
        ...
        # Create CTE
        active_users = cte(
            select(User.id, User.name).where(User.city == 'Moscow'),
            name='active_users'
        )
        ...
        user_orders = select(
            Order.user_id,
            func.count().label('order_count')
        ).group_by(Order.user_id).subquery('user_orders')
        
        # Use both with .c attributes
        from chorm.sql.expression import Identifier
        query = select(
            active_users.c.name,
            user_orders.c.order_count
        ).select_from(Identifier('active_users')).join(
            user_orders,
            active_users.c.id == user_orders.c.user_id

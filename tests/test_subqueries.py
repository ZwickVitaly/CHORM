...
def test_subquery_in_where_in():
    ...
    sql = stmt.to_sql()
    assert "SELECT users.name FROM users WHERE (users.id IN (SELECT orders.user_id FROM orders WHERE (orders.amount > 100)))" in sql
...
def test_subquery_exists():
    ...
    assert "SELECT users.name FROM users WHERE exists(SELECT orders.id FROM orders WHERE (orders.user_id = users.id))" in sql
...
def test_scalar_subquery_in_select():
    ...
    sql = stmt.to_sql()
    assert "SELECT users.name, (SELECT count(orders.id) FROM orders WHERE (orders.user_id = users.id)) AS order_count FROM users" in sql
...
def test_nested_subqueries():
    ...
    sql = stmt.to_sql()
    assert "IN (SELECT users.id FROM users WHERE (users.id IN (SELECT orders.user_id FROM orders WHERE (orders.amount > 1000))))" in sql

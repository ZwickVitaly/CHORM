...
def test_subquery_in_where_clause():
    ...
    # Main query
    query = (
        select(User.name, User.country)
        .select_from(User)
        .where(User.id.in_(high_spenders))
    )
    ...
    query = (
        select(
            ...
            if_(
                ...
                'VIP',
                if_(
                    user_metrics.c.lifetime_value > 1000,
                    'Premium',
                    'Standard'
                )
            ).label('segment')
...
def test_all_features_combined():
    ...
    w = window(
        partition_by=[monthly_totals.c.user_id],
        order_by=[monthly_totals.c.month]
    )

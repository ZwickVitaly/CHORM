...
from chorm.sql.expression import (
    func, uniq, median, quantile, group_array,
    stddev_pop, var_pop, corr,
    to_start_of_month, date_diff, now, today,
    concat, substring, position, length
)
...
def test_date_functions(engine, setup_tables):
    ...
    stmt2 = (
        select(
            Order.date,
            date_diff("day", Order.date, today()).label("days_ago")
        )
        .select_from(Order)
        .limit(1)
    )
...
def test_statistical_functions(engine, setup_tables):
    ...
    stmt2 = (
        select(
            corr(User.age, Order.amount).label("correlation")
        )
        .select_from(User)

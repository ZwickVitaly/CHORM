...
pytestmark = pytest.mark.skipif(
    ...
    reason="CLICKHOUSE_HOST environment variable not set"
)
...
def test_group_bitmap_aggregate(engine, setup_table):
    ...
    stmt = select(
        EventsTable.country,
        group_bitmap(EventsTable.user_id).label("user_bitmap")
    ).select_from(EventsTable).group_by(EventsTable.country)
...
def test_any_last_aggregate(engine, setup_table):
    ...
    stmt = select(
        EventsTable.country,
        any_last(EventsTable.event_type).label("last_event")
    ).select_from(EventsTable).group_by(EventsTable.country)

...
from chorm import Column, MaterializedView, MergeTree, MetaData, Session, Table, create_engine, select
...
class UserEventsMV(Table):
    ...
    __select__ = (
        select(UserEvents.user_id, func.count().label("count")).select_from(UserEvents).group_by(UserEvents.user_id)
    )

...
def test_index_if_not_exists():
    ...
    stmt = add_index(IndexTestTable, "idx_email", Identifier("email"), "bloom_filter", granularity=1, if_not_exists=True)
    sql = stmt.to_sql()
...
def test_multiple_indexes_scenario():
    ...
    idx4 = add_index(IndexTestTable, "idx_desc_ngrams", Identifier("description"), "ngrambf_v1(4, 1024, 3, 0)", granularity=8)

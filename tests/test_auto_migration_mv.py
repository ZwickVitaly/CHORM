...
from chorm import MaterializedView, MergeTree, select
from chorm.auto_migration import MigrationGenerator
...
def test_migration_generator_create_mv():
    ...
    introspector.get_table_info.side_effect = lambda name, db: (
        {"name": "source", "columns": [], "engine": "MergeTree"} if name == "source" else {}
    )

    generator = MigrationGenerator(introspector)

    # Tables in code
    model_tables = {"source": Source.__table__, "view": View.__table__}
